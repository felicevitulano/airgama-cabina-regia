...
1. **Flusso Dati** — dashboard grafica iniziale e **punto di accesso a tutto**: un diagramma "a mappa" (Sistemi esistenti → Agenti Airgama → Dove serve) con **box cliccabili** che portano ai dati e all'analisi di dettaglio (Gestionale→ordini, MES→produzione, App Picking→prelievi, Contabilità→incassi/fido, Cabina, Insight & BI, Riordini, caso agente). In più: **banner di stato colorato** (semaforo complessivo), KPI generali e un **chatbot "Assistente Flussi"** che risponde in linguaggio naturale e ti porta direttamente al dato giusto.
